...
function loadingImage(){
    ...
    return "<img src='https://raw.githubusercontent.com/kubikapp/kubiksoft/master/image/processing.gif' width=300 alt='Loading'>";
...
/** Bonita Function **/
    ...
    function getProcessURL(process,instantiation,isSender){
        ...
        if(instantiation===true){
            ...
            URL = "/bonita/portal/form/process/"+ encodeURI(process.name) +"/" + process.version + "/content/?id=" + process.id;
        }
        
        if(isSender===true){
            URL = URL +"&refUrl="+ window.top.location.href;
        }
    ...
    function cekFile(docType,document){
    	...
    	if(hasDoc){
    		...
    		if(docType == "attachment"){
    		    allowedType = ["application/pdf","image/png","image/jpeg","image/jpg","image/gif","image/svg+xml","image/bmp","image/wbmp","image/tga"];
    		    description = "pdf, png, jpeg, jpg, gif, svg, bmp, wbmp and tga";
    		}
    		    ...
    		    allowedType = ["image/png","image/jpeg","image/jpg","image/gif","image/svg+xml","image/bmp","image/wbmp","image/tga"];
    		    description = "png, jpeg, jpg, gif, svg, bmp, wbmp and tga";
    		}
    		else if(docType == "pdf"){
    		...
    		if(allowedType.indexOf(document.contentType) < 0){
				validation = "File only accepted " + description + ".";
			}
    	}
...
/** Set Icons and Images **/
    ...
    function setKeyIcon(key,operator,condition,iconTrue,iconFalse){
        var condStr =  "",
            iconTrueHtml = "'<i class="+'"'+iconTrue+'"'+"></i>'",
            iconFalseHtml = "'<i class="+'"'+iconFalse+'"'+"></i>'";
        
        if(operator){condStr =  key+operator+"'"+condition+"'"}
        else{condStr =  key}
        
        return condStr+" ? "+iconTrueHtml+" : "+ iconFalseHtml
    }
    
    function setKeyImage(key,operator,condition,picTrue,picFalse){
        var condStr =  "",
            imgTrueUrl = String('"'+"/bonita/API/formsDocumentImage?document="+"'"+'+'+picTrue+'+'+"'"+'"'+"'+"+"'")
            imgTrueHtml = String("'<img src="+imgTrueUrl+' width=25px height=auto>'+"'"),
            imgFalseHtml = String("'<img src="+'"assets/img/'+picFalse+ '" height=25px width=auto>'+"'")
        
        if(operator){condStr =  key+operator+"'"+condition+"'"}
        else{condStr =  key}
        
        return condStr+" ? "+imgTrueHtml+" : "+ imgFalseHtml
    }
    
    function setIcon(condition,iconTrue,iconFalse){
        
        var iconTrueHtml = "<i class="+'"'+iconTrue+'"'+"></i>",
            iconFalseHtml = "<i class="+'"'+iconFalse+'"'+"></i>";
        
        if(condition){return iconTrueHtml}
        else{return iconFalseHtml}
    }
    
     function setImage(condition,picTrue,picFalse,width){
        
        var imgTrueHtml = "<img src="+'"'+"/bonita/API/formsDocumentImage?document="+picTrue+'"'+" width="+width+"%>",
            imgFalseHtml = "<img src="+'"'+"assets/img/"+picFalse+'"'+" width="+width+">";
        
        if(condition){return imgTrueHtml}
        else{return imgFalseHtml}
    }
